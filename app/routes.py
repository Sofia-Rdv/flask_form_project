...
def form():
    ...
    :return: str: Отрендеренный HTML-шаблон 'form.html.'
    """
...
def submit():
    """
    Обрабатывает данные, отправленные пользователем через форму.

    Если метод POST:
    1. Извлекает данные (имя, email, цвет, профессия, хобби, уровень).
    2. Проверяет наличие имени. Если имя пустое, возвращает ошибку 400.
    3. Выбирает случайную шутку и отображает страницу результата.

    Если метод GET:
    Перенаправляет пользователя на главную страницу.

    :return: str | Response: HTML-шаблон результата, ошибки или редирект.
    """
